...
type PbftConsensusNode struct {
	...
	// algorithm running time for a block
	AlgRunTime []float64
}
...
func NewPbftNode(shardID, nodeID uint64, pcc *params.ChainConfig, messageHandleType string) *PbftConsensusNode {
	...
	// init alg runTime array
	p.AlgRunTime = make([]float64, 0)
...
func (p *PbftConsensusNode) closePbft() {
	if p.view == p.NodeID {
		p.writeAlgRunningTime()
	}

	p.CurChain.CloseBlockChain()
}

func (p *PbftConsensusNode) writeAlgRunningTime() {
	...
	targetPath := dirpath + "/Shard" + strconv.Itoa(int(p.ShardID)) + strconv.Itoa(int(p.NodeID)) + "AlgorithmTime.csv"
	...
	title := []string{"blockHeight", "algorithm"}
	w.Write(title)
	w.Flush()
	datanum := 0
	for idx, val := range p.AlgRunTime {
		resultStr := []string{strconv.Itoa(int(idx)), strconv.FormatFloat(val, 'f', 8, 64)}
		datanum++
