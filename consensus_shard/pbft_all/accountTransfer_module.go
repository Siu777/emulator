...
func (cphm *CLPAPbftInsideExtraHandleMod) proposePartition() (bool, *message.Request) {
	...
	txRelay := make([]*core.Transaction, 0)
	txNoRelay := make([]*core.Transaction, 0)
	for _, tx := range cphm.cdm.ReceivedNewTx {
		if tx.Relayed {
			txRelay = append(txRelay, tx)
		} else {
			txNoRelay = append(txNoRelay, tx)
		}
	}
	cphm.pbftNode.CurChain.Txpool.AddTxs2Pool(txNoRelay)
	cphm.pbftNode.CurChain.Txpool.AddTxs2Pool_Head(txRelay)
