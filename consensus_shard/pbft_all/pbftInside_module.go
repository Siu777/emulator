...
func (rphm *RawRelayPbftExtraHandleMod) HandleinPropose() (bool, *message.Request) {
	// new blocks
	block, algorithmTime := rphm.pbftNode.CurChain.GenerateBlock()
	if len(block.Body) != 0 {
		rphm.pbftNode.AlgRunTime = append(rphm.pbftNode.AlgRunTime, algorithmTime)
	}
	r := &message.Request{
...
func (rphm *RawRelayPbftExtraHandleMod) HandleinCommit(cmsg *message.Commit) bool {
	...
	// requestType ...
	// if rphm.pbftNode.CurChain.CurrentBlock.Header.Number == 10 {
	// 	var cmd *exec.Cmd
	// 	cmd.Process.Kill()
	// }
	...
	if rphm.pbftNode.NodeID == rphm.pbftNode.view {
		...
		if len(block.Body) > 0 && rphm.pbftNode.round == -1 {
			rphm.pbftNode.round = 0
		} else if rphm.pbftNode.round >= 0 {
			rphm.pbftNode.round++
		}
		bim := message.BlockInfoMsg{
			BlockBodyLength: len(block.Body),
			ExcutedTxs:      txExcuted,
			Epoch:           rphm.pbftNode.round,
			Relay1Txs:       relay1Txs,
