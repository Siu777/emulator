...
func (bc *BlockChain) GenerateBlock() (*core.Block, float64) {
	// pack the transactions from the txpool
	var beginPackTime, endPackTime time.Time
	beginPackTime = time.Now()
	txs := bc.Txpool.PackTxs(bc.ChainConfig.BlockSize)
	endPackTime = time.Now()
	bh := &core.BlockHeader{
	...
	return b, float64(endPackTime.Sub(beginPackTime).Seconds())
}
